...
const Dashboard = ({ session }: { session: Session }) => {
  ...
  // Hardcoded goals for now, could be moved to user settings later
  ...
  const alerts = useMemo(() => {
    ...
    if (statistics.averageFuelEfficiency > 0 && statistics.averageFuelEfficiency < fuelEfficiencyGoal) {
    ...
    if (statistics.trends && !statistics.trends.isImprovingEfficiency && statistics.trends.efficiencyChange !== 0) {
      ...
      <main className="space-y-8">
        ...
        {statistics && statistics.totalRecords > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-6"><Award className="w-8 h-8 text-purple-600 mb-2" /><p className="text-sm font-medium text-purple-800">総合評価</p><p className="text-2xl font-bold text-purple-900">{alerts.length === 0 ? 'A' : alerts.length <= 1 ? 'B' : 'C'}</p><p className="text-sm text-purple-600">{alerts.length === 0 ? '良好' : '要注意'}</p></div>
            </div>
        )}

        {/* --- Alerts Section --- */}
        {alerts.length > 0 && (
            <div className="bg-white rounded-lg shadow p-6"><h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><AlertCircle className="mr-2 text-orange-500"/>注意事項</h3><div className="space-y-2">{alerts.map((a,i) => <div key={i} className="p-3 rounded-lg bg-orange-50 border border-orange-200 text-orange-800">{a}</div>)}</div></div>
        )}
            ...
            <div className="lg:col-span-2 bg-white p-4 rounded-lg shadow-lg">
                ...
                {records.length > 0 ? (
                            ...
                            <div key={month.month}>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        <thead className="bg-gray-50"><tr className="border-b"><th className="text-left p-2 font-medium">日付</th><th className="text-left p-2 font-medium">スタンド</th><th className="text-right p-2 font-medium">燃費</th><th className="text-right p-2 font-medium">金額</th><th className="text-center p-2 font-medium">操作</th></tr></thead>
                                        <tbody>
                                            {month.records.map(r => (
                                                <tr key={r.id} className="border-b hover:bg-gray-50">
                                                    <td className="p-2">{r.date}</td><td className="p-2"><span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs">{r.station}</span></td><td className="text-right p-2">{r.fuelEfficiency ? formatFuelEfficiency(r.fuelEfficiency) : '-'}</td><td className="text-right p-2">{formatCurrency(r.cost)}</td><td className="text-center p-2"><div className="flex items-center justify-center gap-2"><Button variant="ghost" size="icon" onClick={() => setEditingRecord(r)}><Edit className="w-4 h-4 text-blue-600"/></Button><Button variant="ghost" size="icon" onClick={() => handleDeleteRecord(r.id)}><Trash2 className="w-4 h-4 text-red-600"/></Button></div></td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                    ...
                    <div className="text-center py-16 text-gray-500">
                        <h3 className="text-lg font-semibold">まだ記録がありません</h3>
                        <p className="mt-2">左のフォームから最初の燃費記録を追加してください。</p>
                    </div>
                )}
            </div>
        </div>
